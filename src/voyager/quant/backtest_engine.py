...
class BacktestEngine:
    ...
    def _compute_metrics(
        ...
        # Total return - get portfolio aggregate
        ...
        # For multi-asset portfolio, get portfolio-level volatility
        if isinstance(returns, pd.DataFrame):
            portfolio_returns = returns.sum(axis=1)  # Sum returns across assets
    ...
    def _build_equity_curve(self, portfolio: vbt.Portfolio) -> List[EquityPoint]:
        ...
        # For multi-asset portfolios, sum across assets to get total value
        if isinstance(equity_series, pd.DataFrame):
            equity = equity_series.sum(axis=1)
        else:
            equity = equity_series
        
        # Sample to avoid huge arrays (max 500 points)
        if len(equity) > 500:
            step = (len(equity) + 499) // 500  # Round up to ensure <= 500 points

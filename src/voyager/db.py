...
def get_engine(db_url: Optional[str] = None) -> Engine:
    """
    Return a singleton SQLAlchemy engine for the Voyager database.
...
def _default_schema_path() -> Path:
    """
    Locate sql/voyager_schema.sql relative to this file.
    Assumes project root layout:

      project_root/
        sql/voyager_schema.sql
        src/voyager/db.py
    """
    # db.py → voyager → src → project_root
    project_root = Path(__file__).resolve().parents[2]
    return project_root / "sql" / "voyager_schema.sql"
...
def apply_schema(schema_path: Optional[Path] = None) -> None:
    """
    Apply the core database schema from voyager_schema.sql.
...
def apply_v3_schema() -> None:
    """
    Apply the V3 schema for thesis workflow tables.

    This reads sql/v3_schema.sql and executes it statement-by-statement.
    Creates tables: thesis_snapshot, logic_validation, backtest_result, critique_session
    """
    # Determine path relative to project root
    project_root = Path(__file__).resolve().parents[2]
    schema_path = project_root / "sql" / "v3_schema.sql"

    if not schema_path.exists():
        raise FileNotFoundError(f"V3 schema file not found: {schema_path}")
